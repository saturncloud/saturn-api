        ...
        _obj = cls.model_validate(
            {
                "schema_version": (
                    ...
                    if obj.get("schema_version") is not None
                    else "2025.10.01"
                ),
                "type": obj.get("type"),
                "spec": (
                    DeploymentSpec.from_dict(obj["spec"]) if obj.get("spec") is not None else None
                ),
                "state": (
                    ResourceState.from_dict(obj["state"]) if obj.get("state") is not None else None
                ),
            }
        )
        return _obj
